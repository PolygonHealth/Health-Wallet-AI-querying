...
const FinishWithAnswerSchema = z.object({
  answer: z.string().describe(
    ...
    No tables — use bullet or numbered lists.

    DATA LINKING:
    - Do NOT add citation numbers, source links, reference lists, or footnotes.
    - When referencing patient data, use inline HealthWallet links: [Display Text](healthwallet://RESOURCE/EXACT_NAME).
      -  RESOURCE must be one of relevant resource types in small letters, e.g. condition, medication, encounter, procedure, etc.
      -  EXACT_NAME must match exactly as it appears in the patient data context (case-sensitive, URL-encoded if needed).
...
        - [diabetes history](healthwallet://conditions/Diabetes) `
  ...
  resource_ids: z.array(z.string()).optional().describe('Optional: internal resource IDs (UUIDs) for tracking/analytics. Do NOT include UUIDs in the patient-facing answer.')
});
...
export function createFHIRTools(
  ...
  const finishWithAnswer = tool(
    ...
    },
    {
      ...
      description: 'Always call last. Never return text without calling this. Use HealthWallet markdown links in the answer to link to the patient data.',
...
const extractReferecesToResourceData = tool(
    async (input) => {
      ...
      const repo = _fhirResourcesRepo(dbPool);
      const context = await repo.buildContextNamesOnly(relevantResourceTypes)
      return context;
    },
    {
      name: 'extract_references_to_resource_data',
      description: `Returns all patient resource names and links grouped by category (conditions, medications, etc). 
      Call this tool when you have the relevant resource types and you want to reference exact resource names in your answer. 
      The UI turns exact name matches into clickable links.`,
      schema: ResourceDataLinksSchema
    }
  );
